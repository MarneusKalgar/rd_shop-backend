...
No protocol-level scenario changes are required when switching from local compose to ECS. The stage-validation subset now reads a single `STAGE_VALIDATION_*` contract: `STAGE_VALIDATION_BASE_URL` for target selection, `STAGE_VALIDATION_NAMESPACE` for fixture ownership, and `STAGE_VALIDATION_PRODUCT_ID` / `STAGE_VALIDATION_USER_PASSWORD` for seeded-stage reuse. Once live-stage custom app metrics are enabled in Phase 6.1, stage-validation requests should also send a dedicated header such as `X-RdShop-Traffic-Source: stage-validation-e2e` so the app can mark logs and suppress only custom EMF metrics for that traffic. This does not suppress built-in ALB / ECS / RDS / EC2 metrics.
...
Before relying on custom-metric suppression for live stage validation, normalize stage-validation HTTP requests through a shared helper or shared `supertest` agent that attaches `X-RdShop-Traffic-Source: stage-validation-e2e` to every request. Keep the header value as test-code convention; no extra GitHub Environment variable is required for it.
